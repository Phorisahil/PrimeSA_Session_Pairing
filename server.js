...
function requireInternalApiKey(req, res, next) {
  const configuredKey = process.env.SESSION_API_KEY;
  ...
  if (!checkRateLimit(`internal:${req.ip}`, 30, 60000)) {
...
// Private PrimeSA_Bot API. Never expose these routes in frontend code.
app.get('/api/internal/session/:sessionId/status', requireInternalApiKey, (req, res) => {
  const { sessionId } = req.params;
  if (!validateSessionId(sessionId)) {
    return res.status(404).json({ success: false, error: 'Session not found' });
  }

  const status = sessionManager.getInternalSessionStatus(sessionId);
  if (!status) return res.status(404).json({ success: false, error: 'Session not found' });
  res.set('Cache-Control', 'no-store');
  res.json({ success: true, ...status });
});

app.post('/api/internal/session/:sessionId/claim', requireInternalApiKey, (req, res) => {
  const { sessionId } = req.params;
  if (!validateSessionId(sessionId)) {
    return res.status(404).json({ success: false, error: 'Session not found' });
  }

  const result = sessionManager.claimSession(sessionId, req.get('x-bot-id') || 'PrimeSA_Bot');
  if (result.error === 'not_found') return res.status(404).json({ success: false, error: 'Session not found' });
  if (result.error === 'claimed') return res.status(409).json({ success: false, error: 'Session already claimed' });
  if (result.error === 'invalid') return res.status(410).json({ success: false, error: 'Session expired or no longer valid' });
  if (result.error === 'not_connected') return res.status(409).json({ success: false, error: 'Session is not connected' });

  res.set('Cache-Control', 'no-store');
  logger.info({ sessionId }, 'Session claimed by PrimeSA_Bot');
  res.json({ success: true, sessionId, transferToken: result.transferToken, transferTokenExpiresAt: result.expiresAt });
});

app.post('/api/internal/session/:sessionId/transfer', requireInternalApiKey, (req, res) => {
  const { sessionId } = req.params;
  const transferToken = req.get('x-session-transfer-token');
  const transfer = sessionManager.consumeTransferToken(sessionId, transferToken);

  if (!transfer) return res.status(410).json({ success: false, error: 'Transfer token expired or already used' });
  if (!fs.existsSync(transfer.authStatePath)) return res.status(404).json({ success: false, error: 'Session auth state not found' });
  ...
  const collectFiles = (directory, relativeDirectory = '') => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      else if (entry.isFile()) files.push({ path: relativePath, content: fs.readFileSync(absolutePath).toString('base64') });
    }
  };

  try {
    collectFiles(transfer.authStatePath);
    res.set({ 'Cache-Control': 'no-store, no-cache, must-revalidate', Pragma: 'no-cache' });
    res.json({ success: true, sessionId, format: 'baileys-multi-file-auth-v1', files });
  } catch (error) {
    logger.error({ error, sessionId }, 'Auth state transfer failed');
