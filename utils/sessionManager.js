...
class SessionManager {
      ...
      const connectPairingSocket = () => {
        ...
        socket.ev.on('connection.update', async (update) => {
          ...
          if (authenticated) {
            ...
            this.scheduleAuthenticationNotification(sessionId, socket, state);
          }
  ...
  getAuthStatePath(sessionId) {
    return path.join(this.getSessionPath(sessionId), 'auth_info_baileys');
  }

  sessionExists(sessionId) {
    return Boolean(this.getSession(sessionId));
  }

  getInternalSessionStatus(sessionId) {
    const session = this.getSession(sessionId);
    if (!session) return null;

    const authenticated = Boolean(session.authState?.state?.creds?.registered);
    return {
      sessionId,
  ...
  claimSession(sessionId, claimedBy) {
    const session = this.getSession(sessionId);
    if (!session) return { error: 'not_found' };
    if (session.status === 'expired' || session.status === 'logged_out' || session.status === 'failed') {
      return { error: 'invalid' };
    }
    if (session.claimed) return { error: 'claimed' };
    if (!session.authState?.state?.creds?.registered || session.status !== 'connected') {
    ...
    session.claimedBy = claimedBy || 'PrimeSA_Bot';
    session.transferTokenHash = crypto.createHash('sha256').update(transferToken).digest('hex');
    session.transferTokenExpiresAt = Date.now() + 5 * 60 * 1000;
    session.transferTokenUsed = false;

    return {
      sessionId,
      transferToken,
      expiresAt: session.transferTokenExpiresAt
    };
  }

  consumeTransferToken(sessionId, transferToken) {
    const session = this.getSession(sessionId);
    if (!session || !session.claimed || session.transferTokenUsed) return null;
    if (!session.transferTokenExpiresAt || Date.now() > session.transferTokenExpiresAt) return null;

    const hash = crypto.createHash('sha256').update(transferToken || '').digest('hex');
    if (hash.length !== session.transferTokenHash?.length ||
        !crypto.timingSafeEqual(Buffer.from(hash), Buffer.from(session.transferTokenHash))) return null;

    session.transferTokenUsed = true;
    return {
      authStatePath: this.getAuthStatePath(sessionId),
      sessionId
    };
  }
  ...
  async deleteSession(sessionId) {
    try {
      ...
      if (session?.notificationTimer) {
        clearTimeout(session.notificationTimer);
        session.notificationTimer = null;
      }
  ...
  async cleanupSession(sessionId) {
    try {
      ...
      if (session.expirationTimer) clearTimeout(session.expirationTimer);
      if (session.reconnectTimer) clearTimeout(session.reconnectTimer);
      if (session.notificationTimer) clearTimeout(session.notificationTimer);
    ...
    try {
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
          ...
          session.notificationSent = true;
          session.notificationAttempts = 0;
      ...
      session.notificationAttempts += 1;
      if (session.notificationAttempts < 5 && this.getSession(sessionId)) {
        this.scheduleAuthenticationNotification(sessionId, this.getSession(sessionId).socket, state, 5000);
      } else {
        logger.warn({ sessionId }, 'Authentication notification deferred after retry limit');
      }
    } finally {
  ...
  scheduleAuthenticationNotification(sessionId, socket, state, waitMs = 1000) {
    const session = this.getSession(sessionId);
    if (!session || session.notificationSent || session.notificationTimer) return;

    session.notificationTimer = setTimeout(() => {
      session.notificationTimer = null;
      const currentSession = this.getSession(sessionId);
      if (!currentSession || currentSession.notificationSent) return;
      this.notifyAuthenticatedUser(sessionId, currentSession.socket || socket, state).catch((error) => {
        logger.warn({ error, sessionId }, 'Authentication notification retry failed');
      });
    }, waitMs);
  }
